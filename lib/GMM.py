...
class GMM_model(nn.Module):
    ...
    def forward(self, z, x_hsd):
        ...
        x_hsd = x_hsd.reshape(-1,3,self.args.imagesize,self.args.imagesize)
        ...
        # (-1,256,256,4)
        Norm_d = (D.reshape(-1,self.args.imagesize,self.args.imagesize,1) - M_d.view(-1,1,1, self.num_clusters))**2
        Norm_h = (h.reshape(-1,self.args.imagesize,self.args.imagesize,1) - M_a.view(-1,1,1, self.num_clusters))**2
        Norm_s = (s.reshape(-1,self.args.imagesize,self.args.imagesize,1) - M_b.view(-1,1,1, self.num_clusters))**2
    
        # (-1,4,256,256)
        WSd = torch.mul(gamma, Norm_d.reshape(-1,self.num_clusters,self.args.imagesize,self.args.imagesize))
        WSh = torch.mul(gamma, Norm_h.reshape(-1,self.num_clusters,self.args.imagesize,self.args.imagesize))
        WSs = torch.mul(gamma, Norm_s.reshape(-1,self.num_clusters,self.args.imagesize,self.args.imagesize))
